...
``Contingent Claims`` library. As an example, we will see how the fixed rate bond instrument is
...
The ``Contingent Claims`` tree is not part the template. Instead, it will be created dynamically, as
...
If there is a lifecycle effect (for example a coupon), we will create an Effect for it, which can
then be settled.
...
introduce the concept of an ``Observable``.
