...
-  ``debitControllers``: a set of parties authorizing outgoing transfers
-  ``creditControllers``: a set of parties authorizing incoming transfers

This allows for modeling various controllers of transfers between Alice's and Bob's accounts. For
example:

-  owners-controlled: If the ``owner`` is the sole member of ``debitControllers`` and ``creditControllers`` for
...
-  owner-only-controlled: If, instead, the ``creditControllers`` (of Bob's account) is the empty set, it is
...
-  custodian-controlled: If, as often is the case, the ``custodian`` needs to control what is being
   transferred, we can instead let the ``custodian`` be the sole member of ``debitControllers`` and
   ``creditControllers`` for the accounts.
